        ...
        //наследуемся от стандартной формы
    {
        ...
        public void SetMaxFont() //Функция подгонки размера шрифтов
        {
            foreach (var level in FontLevels) //Проходим по всем "слоям шрифтов"
            {
                ...
                {
                    var font = new Font(SomeElem.Font.Name, maxFont, SomeElem.Font.Style); //Копируем шрифт из данного
                    //элемента но с заданным размерои
                    var temp = TextRenderer.MeasureText(SomeElem.Text,
                        font); //Получаем размер Содержимого для данного шрифта
                    while (SomeElem.Height < temp.Height || SomeElem.Width < temp.Width
                    ) //Пока при данном шрифте размер содержимого больше границ
                    {
                        ...
                        temp = TextRenderer.MeasureText(SomeElem.Text,
            ...
            //получения элемента интерфейса по его имени
        {
            ...
            var answer = Controls.Find(Name, false); //Массив всех элментов
